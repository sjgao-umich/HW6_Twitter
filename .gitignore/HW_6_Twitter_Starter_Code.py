...
def construct_unique_key(baseurl, params):
    ...
    '''
    return f'{baseurl}_{"q"}_{params["q"]}'


def make_request(baseurl, params,auth=oauth):
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    dict
        ...
        JSON
    '''
    if hashtag in CACHE_DICT:
        return make_request(baseurl, {'q':hashtag,'count':count},auth=oauth)
    else:
        CACHE_DICT[hashtag] = make_request(baseurl, {'q':hashtag},auth=oauth)
    return CACHE_DICT[hashtag]
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for item in statuses:
        result = item['entities']['hashtags']
        for a in result:
            str1 = a['text']
            str2 = hashtag.strip('#')
            if sorted(str1.lower()) != sorted(str2.lower()):
                hashtags_list.append(a['text'])
    maxlabel = max(hashtags_list,key=hashtags_list.count)
    #number=hashtags_list.count(max(hashtags_list,key=hashtags_list.count))
    return f'#{maxlabel}'
...
/Users/gaoshijie/Documents/umich/courses/Winter_2021/si507/Homework/HW6_ddl3.29/W21_HW6_Twitter/secrets_starter_code.py
